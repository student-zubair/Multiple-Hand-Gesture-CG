...
## Code Breakdown
1. **Video Capture**: Initializes the webcam to capture video frames.
2. **Hand Detection**: The `HandDetector` module is used to detect hand gestures.
3. **Mouse Control**: Maps the right-hand index finger movement to mouse cursor movement. A specific gesture is used for mouse clicks.
4. **Volume Control**: Tracks the left-hand movement history. A shaking motion either increases or decreases the system volume.
5. **Performance**: The frame rate (FPS) is calculated and displayed on the video feed.
6. **Exit Condition**: The program will terminate if the user presses the 'q' key.
...
3. The program displays the current volume percentage and FPS on the screen.
